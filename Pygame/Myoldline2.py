...
class Score(Actor):
    ...
    #    self.rect.centerx = SCREENRECT.centerx
    #    self.rect.bottom = 10
    #    self.rect.center=SCREENRECT.bottomleft
    #    self.image=Img.score
        self.score_count=0

    def update(self):
        self.score_count=self.score_count+1
        myFont = pygame.font.SysFont("Times New Roman", 30)
        self.image = myFont.render("SCORE:"+str(self.score_count), 1, white)
        if(self.score_count%4==1):
            self.rect=(480,0,150,self.rect[3])
        if(self.score_count%4==2):
            self.rect=(480,400,150,self.rect[3])
        if(self.score_count%4==3):
            self.rect=(40,400,150,self.rect[3])
        if(self.score_count%4==0):
            self.rect=(40,0,150,self.rect[3])
...
class Shot(Actor):
    "The big payload"
    def __init__(self, player,angle,Imgshot):
        Actor.__init__(self, Imgshot)
        self.image=Imgshot
        self.rect.centerx = player.rect.centerx
       # self.rect[2]=self.rect[2]
       # self.rect[3]=self.rect[3]
    ...
    def update(self):
        ...
        #self.rect.right +=self.velocity_x
...
def main():
    ...
    myFont = pygame.font.SysFont("Times New Roman", 30)
    Img.score = myFont.render("SCORE:"+str(score_count), 1, white)
    ...
    while player.alive or explosions:
        ...
        for e in explosions:
            if e.life <= 0:
                explosions.remove(e)
        for s in shots:
            if s.rect.top <= 0 or s.rect[0]<=0 or s.rect[0]>=640:
                shots.remove(s)

        # Move the player
        direction = keystate[K_d] - keystate[K_a]
        shot_direction = shot_direction -(keystate[K_j]-keystate[K_l])
        ...
        if not player.reloading and keystate[K_SPACE] and len(shots) < MAX_SHOTS:
            shotImg=pygame.transform.rotate(Img.shot,angle-90)
            shots.append(Shot(player,angle,shotImg))
